...
// Manager is a center to records all instances live time for a specific function
type Manager struct {
	sync.Locker
	functionName string
...
func (ttl *Manager) clean(ins instance.Instance) {
	ttl.Lock()
	defer ttl.Unlock()
...
func NewTTLManager(functionName string) *Manager {
	ttl := &Manager{
		Locker:       &sync.Mutex{},
...
func (ttl *Manager) ResetInstanceTimer(ins instance.Instance) error {
